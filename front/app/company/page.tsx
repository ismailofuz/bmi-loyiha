...
interface Student {
  id: number; full_name: string; specialty: string;
  email: string; university_name: string; internship_start: string; internship_end: string; status: string;
}
  ...
  return (
    <DashboardShell nav={nav}>
      <div className="space-y-6">
        <PageHeader title="Kompaniya paneli" subtitle="Sizga biriktirilgan talabalar va amaliyot holati" />

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Jami talabalar", value: stat(students.length), color: "bg-amber-50 border-amber-200 text-amber-700" },
            { label: "Faol amaliyot",   value: stat(active),          color: "bg-green-50 border-green-200 text-green-700" },
            { label: "Tugallangan",     value: stat(completed),        color: "bg-blue-50 border-blue-200 text-blue-700" },
          ].map(s => (
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3.5">
            <h2 className="text-sm font-semibold text-gray-700">Talabalarim</h2>
            <Link href="/company/students" className="text-xs font-medium text-[#2563eb] hover:underline">
              Barchasini ko'rish →
            </Link>
          </div>
          {loading ? (
                ...
                <li key={s.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{s.university_name ?? s.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <StatusBadge status={s.status} />
                    <Link href={`/company/reports?student=${s.id}`} className="text-xs font-medium text-[#2563eb] hover:underline">
                      Hisobotlar
                    </Link>
                  </div>
                </li>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Link href="/company/students"
            ...
            <div>
              <p className="text-sm font-semibold text-gray-800">Talabalarim</p>
              <p className="text-xs text-gray-500">Kompaniyaga biriktirilgan talabalar</p>
            </div>
          </Link>
          <Link href="/company/reports"
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Haftalik hisobotlarni ko'rish va baholash</p>
            </div>
          </Link>
          <Link href="/company/applications"
            className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md transition">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">
              <svg viewBox="0 0 20 20" fill="currentColor" className="h-5 w-5"><path fillRule="evenodd" d="M2 3.5A1.5 1.5 0 013.5 2h1.148a1.5 1.5 0 011.465 1.175l.716 3.223a1.5 1.5 0 01-1.052 1.767l-.933.267c-.41.117-.643.555-.48.95a11.542 11.542 0 006.254 6.254c.395.163.833-.07.95-.48l.267-.933a1.5 1.5 0 011.767-1.052l3.223.716A1.5 1.5 0 0118 15.352V16.5a1.5 1.5 0 01-1.5 1.5H15c-1.149 0-2.263-.15-3.326-.43A13.022 13.022 0 012.43 8.326 13.019 13.019 0 012 5V3.5z" clipRule="evenodd" /></svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-800">So'rovlar</p>
              <p className="text-xs text-gray-500">Universitetlardan kelgan amaliyot so'rovlari</p>
            </div>
          </Link>
        </div>
      </div>
